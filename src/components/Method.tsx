...
import { MethodAuroraGrid } from './method/MethodAuroraGrid'
import { CosmicGalaxy } from './method/CosmicGalaxy'
import { CosmicStarfieldCard } from './method/CosmicStarfieldCard'
...
export function Method({ language }: { language: Language }) {
  ...
  useGSAP(() => {
    ...
    mm.add('(min-width: 701px)', () => {
      ...
      gsap.from('.method-hero-galaxy-wrap', { y: 24, opacity: 0, scale: .985, duration: 1, ease: 'power3.out', delay: .1 })
      gsap.from('.method-network', { y: 20, opacity: 0, scale: .985, duration: 1, ease: 'power3.out', delay: .2 })
      gsap.from('.method-companion-card', { y: 20, opacity: 0, duration: 1, ease: 'power3.out', delay: .3 })
  ...
  return (
    <div className="method-stage-wrap">
      {/* Background Aurora Borealis & Google Stitch-style Stardust Dot Matrix */}
      <MethodAuroraGrid interactive={true} />
      ...
      <main ref={root} className="method-page" id="method-panel" role="tabpanel" aria-labelledby="method-tab">
        ...
        <section className="method-hero">
          <div className="method-hero-main">
            <div className="method-hero-copy">
              ...
              <h1 aria-label={t('How Favcolor learns your color', 'Как Favcolor изучает ваш цвет')}>
                {t('It does not guess.', 'Не угадывает.')}<br />
                <em>{t('It learns.', 'Учится.')}</em>
              </h1>
              <p>
                {t(
                  'Every tap updates a small neural model in your browser. Here is what it learned, passed, and failed.',
                  'Каждое нажатие меняет небольшую нейросеть в браузере. Здесь её механизм, результаты и ошибки.'
                )}
              </p>
            </div>
            ...
            <div className="method-hero-galaxy-wrap method-reveal">
              ...
              <div className="method-galaxy-caption">
                <div className="method-galaxy-meta">
          ...
          <div className="method-hero-sidebar">
            <div className="method-network" aria-label={t('Neural architecture: six inputs, two hidden layers, one output', 'Архитектура сети: шесть входов, два скрытых слоя, один выход')}>
              <div className="method-network-head">
                <span>{t('Production model', 'Production-модель')}</span>
                <i>{t('running locally', 'работает локально')}</i>
              </div>
              <div className="method-network-map" aria-hidden="true">
                {[6, 12, 8, 1].map((count, layer) => (
                  <div className="method-network-layer" data-layer={layer} key={count}>
                    <small>{count}</small>
                    {Array.from({ length: count }, (_, index) => <i key={index} />)}
                  </div>
                ))}
              </div>
              <div className="method-network-stats">
                <span><strong>5</strong>{t('networks', 'сетей')}</span>
                <span><strong>197</strong>{t('parameters each', 'параметров в каждой')}</span>
                <span><strong>0</strong>{t('cloud inference', 'облачных запросов')}</span>
              </div>
              <div className="method-model-name">
                <span>{t('Model name', 'Имя модели')}</span>
                <strong>Nikolai Dubovskoy</strong>
                <small>{t('a tribute, not a scientific designation', 'дань уважения, не научное обозначение')}</small>
              </div>
            ...
            >
              <div className="method-companion-body">
                ...
                <div className="method-companion-metrics">
                  <div>
                    <strong>100%</strong>
                    <small>{t('Target recovery', 'Целей найдено')}</small>
                  </div>
                  <div>
                    <strong>0.078</strong>
                    <small>{t('Mean OKLab error', 'Ошибка OKLab')}</small>
                  </div>
                  <div>
                    <strong>0.053</strong>
                    <small>{t('Coverage gap', 'Пробел охвата')}</small>
                  </div>
                </div>
              </div>
            </CosmicStarfieldCard>
        ...
        {/* Process Section: One choice becomes the next question */}
        <section className="method-process method-reveal" id="method-process">
          <header>
            <h2>{t('One choice becomes the next question.', 'Один выбор становится следующим вопросом.')}</h2>
          </header>
          <div className="method-process-layout">
            <div className="method-formula">
              <span>{t('Pair probability', 'Вероятность выбора')}</span>
              <strong>p(A) = σ(u(A) - u(B))</strong>
              <p>
                {t(
                  'The model learns relative preference. It never asks you to score a color on an abstract scale.',
                  'Модель учит относительное предпочтение. Она не просит оценивать цвет по абстрактной шкале.'
                )}
              </p>
            </div>
        ...
        {/* Evidence Section: Numbers that can say no */}
        <section className="method-evidence method-reveal" id="method-evidence">
          <header>
            <h2>{t('Numbers that can say no.', 'Числа, которые могут сказать «нет».')}</h2>
            <p>
              {t(
                'Every number comes from a committed deterministic artifact. Live user metrics are not presented as scientific proof.',
                'Каждое число взято из сохранённого детерминированного артефакта. Метрики пользователей не выдаются за научное доказательство.'
              )}
            </p>
          </header>
          <div className="method-evidence-board">
            <article className="method-evidence-lead">
              <span>{t('Recovery after a noisy start', 'Выход из шумного старта')}</span>
              <strong>8 / 8</strong>
              <p>{t('hidden targets recovered by 150 clicks', 'скрытых целей найдено к 150 выборам')}</p>
              <div className="method-spark" aria-hidden="true">
                {[31, 44, 39, 56, 49, 68, 73, 100].map((height, index) => (
                  <i key={index} style={{ height: `${height}%` }} />
                ))}
              </div>
              <small>{t('red · purple · cyan · green', 'красный · фиолетовый · голубой · зелёный')}</small>
            </article>
            <div className="method-comparison">
              <div className="method-comparison-head">
                <span>{t('Metric', 'Метрика')}</span>
                <span>{t('Before', 'До')}</span>
                <span>{t('After', 'После')}</span>
              </div>
              {recoveryRows.map(row => (
                <div className="method-data-row" key={row.label[0]}>
                  <span>{language === 'ru' ? row.label[1] : row.label[0]}</span>
                  <s>{row.before}</s>
          ...
          <p className="method-protocol">
            {t(
              'Protocol: 4 hidden targets × 2 deterministic seeds, 20 noisy early answers, 150 online test-then-train clicks. Synthetic users are software tests, not evidence about human psychology.',
              'Протокол: 4 скрытые цели × 2 детерминированных seed, 20 ранних шумных ответов, 150 циклов test-then-train. Синтетические пользователи - это тесты программы, а не доказательство о психологии людей.'
            )}
          </p>
          <div className="method-checkpoint">
            <div className="method-checkpoint-copy">
              <h3>{t('Open the run at a checkpoint', 'Откройте прогон в контрольной точке')}</h3>
              <p>{t('The optimum did not improve in a straight line. The test keeps that visible.', 'Оптимум улучшался не по прямой. Тест это не скрывает.')}</p>
            </div>
            <div className="method-checkpoint-controls" aria-label={t('Benchmark checkpoint', 'Контрольная точка теста')}>
              {([50, 100, 150] as const).map(value => (
                <button key={value} type="button" aria-pressed={checkpoint === value} onClick={() => setCheckpoint(value)}>
                  {value}
                </button>
              ))}
            </div>
            <div className="method-checkpoint-values" key={checkpoint}>
              <span><strong>{checkpoints[checkpoint].error}</strong>{t('optimum error', 'ошибка оптимума')}</span>
              <span><strong>{checkpoints[checkpoint].loss}</strong>log-loss</span>
              <span><strong>{checkpoints[checkpoint].coverage}</strong>{t('gamut coverage', 'охват gamut')}</span>
            </div>
          </div>
        </section>
        ...
        <section className="method-integrity method-reveal">
          ...
          <div className="method-integrity-grid">
            <article className="method-integrity-card method-integrity-gray">
              <span>{t('Gray endpoints after warm-up', 'Серые варианты после разогрева')}</span>
              <div><s>22.18%</s><strong>0.49%</strong></div>
              <p>{t('Hue features used to stay loud even when chroma was zero.', 'Признаки оттенка раньше оставались сильными даже при нулевой насыщенности.')}</p>
            </article>
            <article className="method-integrity-card">
              <span>{t('Colorful final optima', 'Цветные финальные оптимумы')}</span>
              <div><s>7 / 12</s><strong>12 / 12</strong></div>
              <p>{t('Same 12 seeds and 100-click protocol.', 'Те же 12 seed и протокол на 100 выборов.')}</p>
            </article>
            <article className="method-integrity-card method-integrity-targets">
              <div className="method-target-head">
                <span>{t('Optimum error by target', 'Ошибка оптимума по цели')}</span>
                <small>{t('lower is better', 'меньше лучше')}</small>
              </div>
              {targetRows.map(row => (
                <div key={row.label}>
                  <b>{row.label}</b>
                  <s>{row.before}</s>
                  <strong>{row.after}</strong>
                  <em className={row.regressed ? 'is-regression' : ''}>{row.change}</em>
                </div>
              ))}
              <p>{t('Regressions stay visible. Aggregate improvement is not permission to hide them.', 'Регрессии не спрятаны. Общее улучшение не даёт права их скрывать.')}</p>
            </article>
            <article className="method-integrity-card method-integrity-gates">
              <div><strong>0 / 16</strong><span>{t('false context activations', 'ложных включений контекста')}</span></div>
        ...
        <section className="method-candidate method-reveal">
          <div className="method-candidate-copy">
            <h2>{t('A model that did not ship.', 'Модель, которая не вышла.')}</h2>
            <p>{t('A compact neural candidate was evaluated on anonymous opt-in packets. It did not beat production, so it stayed an experiment.', 'Компактный нейронный кандидат проверили на добровольно отправленных анонимных пакетах. Он не превзошёл production и остался экспериментом.')}</p>
            <div className="method-candidate-volume">
              <span><strong>1,092</strong>{t('raw observations', 'сырых наблюдения')}</span>
              <span><strong>994</strong>{t('usable', 'пригодных')}</span>
              <span><strong>23</strong>{t('packets', 'пакета')}</span>
            </div>
          </div>
          <div className="method-candidate-table">
            <div>
              <span>{t('Grouped estimate', 'Групповая оценка')}</span>
              <b>{t('Production', 'Production')}</b>
              <b>{t('Candidate', 'Кандидат')}</b>
            </div>
        ...
        <section className="method-history method-reveal" id="method-history">
          <header>
            <h2 aria-label={t('Development history', 'История разработки')}>{t('Built by correcting itself.', 'Сделано через исправление ошибок.')}</h2>
          </header>
          <div className="method-history-list">
            {history.map(([date, title, body], index) => (
              <article className="method-history-item" key={`${date}-${title}`}>
                <time dateTime={`2026-09-${date.slice(0, 2)}`}>{date}.2026</time>
        ...
        {/* Boundary & Philosophy Section */}
        <section className="method-boundary method-reveal" id="method-boundary">
          ...
          <div className="method-boundary-notes">
            ...
            <p>{t('The production model is a real neural network trained only from your choices. Benchmarks test software recovery, not people.', 'Production-модель - настоящая нейросеть, которая учится только на ваших выборах. Benchmark проверяют программу, а не людей.')}</p>
            <p>{t('Your model and history stay in this browser. Anonymous research sharing remains strictly opt-in.', 'Модель и история остаются в браузере. Анонимная отправка данных для исследований включается только добровольно.')}</p>
          </div>
        </section>
      </main>
    </div>
