...
export const isNewChoiceMilestone = (previous: number, current: number) => current >= 100 && current % 100 === 0 && current === previous + 1
...
export function Discover({ model, language, showReadyResult = false, onOpenResult = () => {} }: { model: ReturnTypeOfColorModel; language: Language; showReadyResult?: boolean; onOpenResult?: () => void }) {
  ...
  const previousChoiceCount = useRef(committedChoiceCount)
  useEffect(() => {
    const previous = previousChoiceCount.current
    previousChoiceCount.current = committedChoiceCount
    if (!isNewChoiceMilestone(previous, committedChoiceCount) || window.matchMedia?.('(prefers-reduced-motion: reduce)').matches) return
    setCelebrating(true)
    const timeout = window.setTimeout(() => setCelebrating(false), 850)
  ...
  return <main className="discover" id="discover-panel" role="tabpanel" aria-labelledby="discover-tab">
    <section className="discover-copy">
      ...
      <div className="study-status"><span className="status-kicker"><i />{stage}</span><span className="choice-count" key={committedChoiceCount}><strong>{committedChoiceCount}</strong> {answerLabel}</span>{celebrating && <span className="milestone-confetti" data-testid="milestone-confetti" aria-hidden="true">{Array.from({ length: 12 }, (_, index) => <i key={index} style={{ '--confetti-color': index % 3 === 2 ? '#b99aff' : colorCss(model.pair.displayed[index % 2]) } as CSSProperties} />)}</span>}</div>
    </section>
