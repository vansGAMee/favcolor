...
const event: ChoiceEvent = {
  ...
  predictedProbabilityBeforeChoice: .62, estimatedOptimumBeforeChoice: { l: .6, c: .1, h: 20 }, modelStateBeforeChoice: 'Learning',
}
...
describe('IndexedDB persistence', () => {
  ...
  it('exports, imports, and resets schema-versioned data', async () => {
    ...
    expect(JSON.parse(json).choices[0].predictedProbabilityBeforeChoice).toBe(.62)
    await db.reset()
