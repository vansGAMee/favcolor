...
export function assessReadiness(choices: ChoiceEvent[], metrics: ValidationMetrics | null, spread: number) {
  const controls = choices.filter(choice => choice.pairType === 'repeated-control')
  ...
  if (
    ...
    challenges.length >= 1 && (challengeWinRate ?? 0) >= 0.5
  ) state = 'Ready'
  return { state, controlCount: controls.length, controlConsistency, challengeCount: challenges.length, challengeWinRate }
}
