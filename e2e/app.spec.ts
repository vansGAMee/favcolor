...
test('mobile 390px keeps equal cards, keyboard flow, and no overflow', async ({ page }) => {
  ...
  await page.getByRole('tab', { name: 'You' }).click()
  await assertNoOverflow(page)
  await expect(page.getByText('Past 13 weeks')).toBeVisible()
  expect(await page.locator('.history-scroll').evaluate(element => element.scrollWidth <= element.clientWidth)).toBe(true)
  ...
  expect(historyDay?.width).toBeGreaterThanOrEqual(20)
  expect(historyDay?.height).toBeGreaterThanOrEqual(20)
...
test('desktop analytics composition has no horizontal overflow', async ({ page }) => {
  ...
  await expect(page.getByText('Past 26 weeks')).toBeVisible()
  await assertNoOverflow(page)
  await page.waitForTimeout(700)
